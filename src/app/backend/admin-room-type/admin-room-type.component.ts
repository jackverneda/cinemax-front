...
import { DialogAddEditRoomTypeComponent } from './dialog-add-edit-room-type/dialog-add-edit-room-type.component';
...
@Component({
  ...
  styleUrls: ['./admin-room-type.component.scss'],
})
export class AdminRoomTypeComponent implements OnInit, OnDestroy {
  allRoomTypes: any;
  ...
  onCreateRoomType(): void {
    let dialogRef: MatDialogRef<DialogAddEditRoomTypeComponent, any>;
    dialogRef = this.dialog.open(DialogAddEditRoomTypeComponent, {
  ...
  onEditRoomType(roomType: any): void {
    this.roomTypeService.get(roomType.id).subscribe(
      (data: any) => {
        let dialogRef: MatDialogRef<DialogAddEditRoomTypeComponent, any>;
        dialogRef = this.dialog.open(DialogAddEditRoomTypeComponent, {
          panelClass: 'app-dialog-add-edit-about-us',
  ...
  async onRemoveRoomTypes(elements: any[]) {
    ...
    dialogRef.afterClosed().subscribe(async (result) => {
      try {
        if (result) {
          await Promise.all(elements.map((item) => this.roomTypeService.delete(item.id).toPromise()));
