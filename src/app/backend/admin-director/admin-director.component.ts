...
export class AdminDirectorComponent implements OnInit, OnDestroy {
  allDirectors: any;
  ...
  onCreateDirector(): void {
    let dialogRef: MatDialogRef<DialogAddEditDirectorComponent, any>;
    dialogRef = this.dialog.open(DialogAddEditDirectorComponent, {
  ...
  onEditDirector(director: any): void {
    this.directorService.get(director.id).subscribe(
      (data: any) => {
        let dialogRef: MatDialogRef<DialogAddEditDirectorComponent, any>;
        dialogRef = this.dialog.open(DialogAddEditDirectorComponent, {
          panelClass: 'app-dialog-add-edit-about-us',
  ...
  async onRemoveDirectors(elements: any[]) {
    ...
    dialogRef.afterClosed().subscribe(async (result) => {
      try {
        if (result) {
          await Promise.all(elements.map((item) => this.directorService.delete(item.id).toPromise()));
