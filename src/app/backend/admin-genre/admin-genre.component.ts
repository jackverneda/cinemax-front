...
export class AdminGenreComponent implements OnInit, OnDestroy {
  allGenres: any;
  ...
  onCreateGenre(): void {
    let dialogRef: MatDialogRef<DialogAddEditGenreComponent, any>;
    dialogRef = this.dialog.open(DialogAddEditGenreComponent, {
  ...
  onEditGenre(genre: any): void {
    this.genreService.get(genre.id).subscribe(
      (data: any) => {
        let dialogRef: MatDialogRef<DialogAddEditGenreComponent, any>;
        dialogRef = this.dialog.open(DialogAddEditGenreComponent, {
          panelClass: 'app-dialog-add-edit-about-us',
  ...
  async onRemoveGenres(elements: any[]) {
    ...
    dialogRef.afterClosed().subscribe(async (result) => {
      try {
        if (result) {
          await Promise.all(elements.map((item) => this.genreService.delete(item.id).toPromise()));
